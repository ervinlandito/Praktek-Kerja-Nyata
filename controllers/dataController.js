  ...
  try {
    ...
    try{
      dataRealiasi=await axios.post(`${process.env.URL_API}/data-realisasi`, {
        ...
        logOperation({
          ...
          details: `Successfully get data from ${process.env.URL_API} for date ${tanggal}`
        });
    }catch(error){
        ...
        logOperation({
          status: 'get error',
          timestamp: moment().tz('Asia/Jakarta').format(),
          details: `Failed get data from ${process.env.URL_API} for date ${tanggal}`
        });
    }
    ...
    if (files.length > 30) {
      const sortedFiles = files.map(file => ({
        name: file,
        time: fs.statSync(path.join(directoryPath, file)).ctime.getTime()
      })).sort((a, b) => a.time - b.time);

      while (sortedFiles.length > 30) {
        const oldestFile = sortedFiles.shift();
        fs.unlinkSync(path.join(directoryPath, oldestFile.name));
      }
    }
    ...
    logOperation({
      status: 'success',
      timestamp: moment().tz('Asia/Jakarta').format(),
      details: `Data successfully retrieved and saved on db for date ${tanggal}`
    });
    }catch(error){
      logOperation({
        status: 'error',
    ...
    logOperation({
      status: 'error',
      timestamp: moment().tz('Asia/Jakarta').format(),
      details: `Data cannot retrieved : ${error.message}`
    });
  }
}
...
const logOperation = (logEntry) => {
  ...
  if (logFiles.length > 30) {
    const sortedLogFiles = logFiles.map(file => ({
      name: file,
